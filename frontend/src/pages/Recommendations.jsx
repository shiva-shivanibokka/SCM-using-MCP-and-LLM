...
const GLOSSARY = [
  ...
  { term: "Basket / order", what: "One shopping trip — a single order that can contain several products. Real baskets (not guesses) are what make these recommendations trustworthy." },
  { term: "How it's built", what: "A dbt + DuckDB mart pre-computes every pairing from the full order history, so the recommendation is an instant lookup no matter how large the data grows." },
]
  ...
  return (
    <div>
      <PageHeader
        ...
        blurb="Which products customers buy together, learned from real order baskets. The signal behind cross-sell, bundles, and 'customers who bought X also bought Y'. Pre-computed by a dbt + DuckDB mart, so it stays instant as the data grows."
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard
          index={0}
          title="Product pairings found"
          value={num(data?.total_pairs ?? 0)}
          accent="teal"
          emoji="🔗"
          help="Distinct product pairs that are bought together often enough to recommend."
        />
        <KpiCard
          index={1}
          title="Strongest pairing"
          value={num(data?.max_co_purchases ?? 0)}
          accent="pink"
          emoji="⭐"
          help="Co-purchase count of the single most frequently bought-together pair."
        />
        <KpiCard
          index={2}
          title="Top pair"
          ...
          accent="sky"
          emoji="🛒"
          help="The two products most often bought in the same order."
        />
        <KpiCard
          index={3}
          title="Source"
          ...
          accent="grape"
          emoji="⚙️"
          help="Whether these came from the pre-built dbt + DuckDB mart or were computed on the fly from raw orders."
        />
      </div>

      <ChartCard
        title="Top frequently-bought-together pairs"
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={Math.max(320, chart.length * 40)}>
            <BarChart data={chart} layout="vertical" margin={{ left: 24, right: 32 }}>
              ...
              <Tooltip
                formatter={(v) => [`${v.toLocaleString("en-IN")} orders`, "Co-purchases"]}
